...
const ROLES = ["PERSONERO", "CONTRALOR"];
...
export default function AdminPanelScreen({ navigation }) {
  ...
  const onAdd = async () => {
    try {
      ...
      if (!imageUri) {
        Alert.alert("Falta info", "Elige una imagen.");
        return;
      }

      await addCandidate({ name, imageUri, role });
        ...
        <View style={styles.roleRow}>
          {ROLES.map((r) => (
            <Pressable
              ...
              style={[
                styles.rolePill,
                role === r ? styles.rolePillActive : null,
              ]}
            >
              <Text style={[styles.roleText, role === r ? styles.roleTextActive : null]}>
                {r}
              </Text>
            </Pressable>
          ))}
...
const styles = StyleSheet.create({
  ...
  logoutBtn: { paddingVertical: 10, paddingHorizontal: 12, borderRadius: 12, backgroundColor: "#111" },
  ...
  rolePill: { paddingVertical: 8, paddingHorizontal: 10, borderRadius: 999, backgroundColor: "#f1f1f4" },
  rolePillActive: { backgroundColor: "#111" },
  roleText: { fontWeight: "900", opacity: 0.7 },
  roleTextActive: { color: "#fff", opacity: 1 },
